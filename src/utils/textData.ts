...
const getRandomText = (difficulty: 'easy' | 'medium' | 'hard'): { id: string; text: string } => {
  const texts = textData[difficulty]
  if (!texts || texts.length === 0) {
    throw new Error(`No texts available for difficulty: ${difficulty}`)
  }
